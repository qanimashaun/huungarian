...
public class ProjectMatcherUpgrade{
  ...
  public static void main(String[] args) {
    if(args.length<2){
      ...
      int counter = 0;
      int minLines = getMinLines(costMatrix);
      ...
      while(minLines!=n){
        ...
        printMatrix(costMatrix);
         minLines = getMinLines(costMatrix);
  ...
  public static int getMinLines(int[][] matrix){
    ...
    for(int i = 0; i< rowsIndexCovered.length; i++){
      for(int j = 0; j<coloumnsIndexCovered.length; j++){
        if(matrix[i][j] == 0 && rowsIndexCovered[i] == 0 && coloumnsIndexCovered[j] ==0){
  ...
  public static int getDirection(int[][] matrix, int row, int coloumn){
    ...
    for(int i=0; i< matrix.length; i++){
      if(matrix[i][coloumn]==0 && rowsIndexCovered[i] == 0){
        coloumnCounter++;
      }
    }
    for(int i =0; i<matrix[row].length; i++){
      if(matrix[row][i]==0 && coloumnsIndexCovered[i] == 0){
        rowCounter++;
      }
    }
